	...
	Model.prototype.initMonth=function(){
		...
		eventArr.forEach(function(val){
			...
			if((firstDay <=val.theTime) && (val.theTime <=lastDay) && (rowLine !==undefined)){
				...
				var eventIcon = $("<i><span class='the_title' title='"+spanInfo+"'>"+ (val.eventType).split("")[0] +"</span></i>").appendTo( $("tbody tr").eq(rowLine).find("td").eq(index -0+1) );
				...
				if(val.status.split("")[1] ==2) {
					eventIcon.css("color", "#e04df2");
				}else{
					eventIcon.css("color", "#5370ff");
				}
			}
		});
				...
				if (data.__statusCode=='S'){
					...
					activity_data.forEach(function(val){
						if(val.actiivityStatus){	//必须要有活动状态，
							...
							eventArr.push({
								"theTime": val.actiivityStatusDate.split(" ")[0],
