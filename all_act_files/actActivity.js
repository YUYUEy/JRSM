define(function(require){
	...
	//
	function getShowData(year, month){
		 var monthData2 =( datepicker2.getMonthData(year,month));
		 if(monthData2[91].showDate <10){
		 	monthData2.splice(91,7);
		 }
		 if(monthData2[84].showDate <10){
		 	monthData2.splice(84,7);
		 }
		 return monthData2;
	}
	function getFirstAndLastDay(){		//获取当前页面的时间范围，开始日和结束日的具体日期
		year = mGetDate(year, month).split('-')[0];
		month = mGetDate(year, month).split('-')[1];
		var monthData = getShowData(year, month);
		firstDay = format(new Date(monthData[0].year, monthData[0].month - 1, monthData[0].showDate));	//开始日的具体日期
		last_Day = GetDateStr(firstDay, monthData.length);		//结束日的具体日期
		return [firstDay , last_Day];
	}
	//
	function initMonth() {
		year = mGetDate(year, month).split('-')[0];
		...
		$("#wrap").html(datepicker2.buildUi(year, month,showShopArr,initShopArr));
	...
	//6种状态
	var statusObj =[
	      "<i class='icon iconfont icon-triangle-copy' style='color:#5370ff;'></i>", "<i class='icon iconfont icon-triangle-copy' style='color:#e04df2;'></i>",
	      "<i class='icon iconfont icon-yuanxing' style='color:#5370ff;'></i>", "<i class='icon iconfont icon-yuanxing' style='color:#e04df2;'></i>",
	      "<i class='icon iconfont icon-xingxing' style='color:#5370ff;'></i>", "<i class='icon iconfont icon-xingxing' style='color:#e04df2;'></i>"
	];
	...
	function showThreeMonth(){
		...
		eventArr.forEach(function(val){
			if(((firstDay<= val.startTime)&&(val.startTime<= last_Day)) || ((firstDay<= val.endTime)&&(val.endTime<= last_Day))){
				...
				var initWidth = parseInt((Date.parse( val.endTime )- Date.parse( val.startTime ))/(1000* 3600* 24)) *10 +10 +'px';	//初始宽度
				var iii;
				["A1", "A2", "B1", "B2", "C1", "C2"].forEach(function(value, index){
					if(val.actiivityStatus ==value) iii = $(statusObj[index]);
				});
				if(initTop && initLeft){
					var p = $("<p class='showEvent' data-allinfo='"+ JSON.stringify(val) +"'><span class='showEventSpan' title='"+val.startTime+'~'+ val.endTime +'; '+ val.smallCatalogName + ': ' + val.str+"'>"+ val.smallCatalogName + ':' + val.str +"</span><a class='to_copy'>+</a><a class='to_del'>-</a></p>").appendTo($(".bor"));
		...
		$(this.getRootNode()).on("click",function(e){
			if((e.target.className=="bor cjE7VJb")){		//点击空白处增加活动事件
				...
				_this.comp("activityAddView").open({
					data : {operator : "add"},
					title :	messagehelper.get("act.activity.title"),
					params:{
						postArr: postArr,
						shopId: userStore[0],
						...
						the_start_time: GetDateStr(firstDay, Math.ceil(e.offsetX /10) )
					}
				});
				...
				if( allInfo.dataFrom =="event" ){
					dialoghelper.info(messagehelper.get("msgDial.activity.I005_1"));		//总部促销活动录入的不能修改
					return;
				}
...
//				}
				...
				_this.comp("activityAddView").open({
					data : {operator : "add"},
					title : messagehelper.get("act.activity.title2"),
					params:{
						 postArr: postArr,
						 rowId:editID,
					}
			...
			}else if((e.target.className =="to_copy cjE7VJb") ){	//弹出复写窗口
				allInfo = JSON.parse(e.target.parentNode.dataset.allinfo);
				if(allInfo.chargerCode !=userInfo_id ){
					dialoghelper.info(messagehelper.get("msgDial.activity.I004"));
					return;
				...
				editID = allInfo.code;
				disX = parseInt(e.target.parentNode.style.left);
				disY = parseInt(e.target.parentNode.style.top);
				var spanWidth =parseInt(e.target.parentNode.style.width);
				...
				if( allInfo.dataFrom =="event" ){
					dialoghelper.info(messagehelper.get("msgDial.activity.I003_1"));		//总部促销活动录入的不能删除
				}else if( allInfo.dataFrom =="BATCH" ){										
					if(editStatus.substr(0,1) =="A" || editStatus.substr(0,1) =="B" || editStatus.substr(0,1)=="C"){
						dialoghelper.info(messagehelper.get("msgDial.activity.I001"));		//总部一括录入的，已有活动状态的不能删除
					}else{																	//总部一括录入的，提醒是否删除？
						dialoghelper.confirmMsg(messagehelper.get("msgDial.submitDelete2"),_this2.deleteRow,null,null,"actActivity",editID);
					}
				}else{
					if(editStatus.substr(0,1) =="A" || editStatus.substr(0,1) =="B" || editStatus.substr(0,1)=="C"){
						dialoghelper.info(messagehelper.get("msgDial.activity.I001"));		//自由录入的，已有活动状态的不能删除
					}else{
				...
				_this.comp("activityTypeView").open({
					title :	messagehelper.get("act.activity.title3") + bigTypeArr[1],
					params:{
	...
	Model.prototype.button21Click = function(event){	//打开一括登录子页面
		var options = {
			title : messagehelper.get("msgDial.activityBatch"),
			url : "$UI/jrsm/module/act/actActivityBatch.w",
			close : true		// 是否允许关闭
		};
	...
	Model.prototype.modelLoad = function(event) {
		...
		queryFlag = 0;
		_this2 = this;
		year = new Date(serverTime).getFullYear(); 
		month = new Date(serverTime).getMonth() +1;
		this.queryShop();		//查询并挂载复写元和复写先的店铺
				...
				(Data.data).forEach(function(val){
					if(val.typeCatalog =="D"){
						...
						if(val.processFlag =="N"){
							...
							li.find("button").css({"background": val.typeColor.colorRgb() });
						}else{
							var li2 = $("<li><button class='bigTypeName' data-info='"+ arr1 +"'>"+ val.typeName +"+</button></li>").appendTo( $("#selectGroup2") );
							li2.find("button").css({"background": val.typeColor.colorRgb() });
						}
					}
				});
			},
	...
	Model.prototype.queryActivity = function(){
		...
		console.log((global.getProfile()).__userName);
		...
		var data = {
				__pagesize: pageSize,
				__page: page,
				queryDateFrom: firstDay,
				queryDateTo: last_Day
			};
		...
		global.ajax({
			...
			success: function(data){
				if (data.__statusCode=='S'){
					console.log( data);
					var userStore_data = data.data.userStoreList;		//获取店铺信息
					var activity_data = data.data.activityList;			//获取施策计划信息
					...
					initShopArr.forEach(function(val){		//初始化时挂载复写至目标店铺
						val.createdBy = userStore_data[0].chargerCode;
						val.createdByName = userStore_data[0].chargerName;
					});
					showShopArr = unique2(showShopArr);		//去除重复
					console.log(showShopArr);
					showUserArr = userArr;
	...
	Model.prototype.copyAll = function(event){
		...
		global.ajax({
			...
			data:{
				...
				"endDate": the_endDate,
				"storeCode" : JSON.stringify({"fromStoreCode":$("#start_copy").val(), "toStoreCode":endCopy})
			},
			async:false,
			success: function(Data){
				_this.queryActivity();
...
//		data.storeCode = storeCode;
		...
		data.__page = page;
		data.queryDateFrom= firstDay;
		data.queryDateTo= last_Day;
...
//					console.log(showShopArr);
	...
	Model.prototype.gridSelect1ShowOption = function(event){	//设置复写先下拉框的菜单向上展开
		...
		setTimeout(function(){
			$(".x-gridSelect-option").css("top", (grid_top ) +'px');
		}, 30);
	};
